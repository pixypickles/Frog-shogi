(() => {
  ...
  function fighterPalette(type){
    if(type==='awakenedKokabiel') return {body:'#d6b83f',limb:'#9f8124',light:'#70dce9',belly:'#bceff3',eyeBump:'#70dce9'};
  ...
  const ANGEL_TYPES=new Set(['green','blue','yellow','orange','remiel','jihal','seraphiel','kawazu']);
  const DEMON_TYPES=new Set(['black','purple','beelzebub','samael','sariel','kokabiel','awakenedKokabiel','flauros','satanael']);

  function defaultWingKind(type){
    if(DEMON_TYPES.has(type))return 'demon';
    return 'angel';
  }

  function drawSkyWings(f){
  ...
  class Fighter {
    constructor(x, y, isPlayer, type='green') {
      ...
      // 雲上格闘2：毒状態。ベルゼブブ（将来のサマエルも）は継続毒を無効化。
      this.poisonT=0;
      this.poisonTick=0;
    ...
    draw() {
      ...
      if(this.type==='crayfish'){
        ...
        if(this.type==='awakenedKokabiel'){
        ctx.save();ctx.globalCompositeOperation='lighter';
        ctx.globalAlpha=.24+.07*Math.sin(performance.now()/120);
        ctx.strokeStyle='#78e7ef';ctx.lineWidth=3;ctx.shadowColor='#55dce8';ctx.shadowBlur=18;
        ctx.beginPath();ctx.ellipse(0,16,38,49,0,0,Math.PI*2);ctx.stroke();
        ctx.globalAlpha=.18;ctx.fillStyle='#7be9ef';ctx.beginPath();ctx.arc(-22,-27,13,0,Math.PI*2);ctx.fill();
        ctx.beginPath();ctx.arc(22,-27,13,0,Math.PI*2);ctx.fill();ctx.restore();
      }

      if((this.type==='kokabiel'||this.type==='awakenedKokabiel')&&(this.specialType==='gravityBall'||this.specialType==='gravityZone'||this.specialType==='meteorRain')){
      ...
      if((this.type==='kokabiel'||this.type==='awakenedKokabiel')&&this.specialType==='gravityDive'&&this.specialT>0){
      ...
      if(this.tongueT>0 || (this.tonguePullTarget && this.tonguePullTimer>0) || (this.tongueClashTarget && this.tongueClashTimer>0)){
        ...
        if(target){
          // 描画座標は face で左右反転済みなので、相手方向をローカル座標に変換。
          const worldDx=target.x-this.x;
          const worldDy=target.y-this.y;
          const localDx=worldDx*(this.face||1);
          const dist=Math.hypot(localDx,worldDy)||1;
          const reach=Math.min(this.tongueRange,dist);
          const tx=localDx/dist*reach;
          const ty=worldDy/dist*reach;
  ...
  function practiceSpecialText(type){
    const map={
      ...
      blue:['上 ＋ ガード：ウォータージェット上昇','ジェット上昇中 ＋ パンチ：ジェットアッパー','上 ＋ パンチ：ビッグウォータードロップ','後ろ ＋ キック：クラウドレインショット','下 ＋ キック：スカイレインバースト','下 → 前 ＋ パンチ：ギガウォーターキャノン（反動あり）'],
      yellow:['方向 ＋ パンチ：エアカッター（逆方向へ逃げながら発射・最大45°自動補正）','方向 ＋ キック：ウィンドタックル','ガード ×2：ヒールウィンド（徐々に回復＋速度アップ）','隠し技・グランドトルネード：方向キー1回転 ＋ パンチ（クールタイム10秒）'],
  ...
  function mixTypeFor(nameOrType){
    ...
    const map={
      ...
      'ベルゼブブ':'beelzebub','ベルゼブブさん':'beelzebub','サマエル':'samael','サマエルさん':'samael','セラフィエル':'seraphiel','セラフィエルさん':'seraphiel','レミエル':'remiel','レミエルさん':'remiel','ジィハル':'jihal','ジィハルさん':'jihal','コカビエル':'kokabiel','コカビエルさん':'kokabiel','覚醒コカビエル':'awakenedKokabiel','覚醒コカビエルさん':'awakenedKokabiel','サリエル':'sariel','サリエルさん':'sariel',
  ...
  function specialRaphaelBubbleMove(f,verticalDir=null){
    if(gameOver || f.stun>0 || f.throwState || f.specialT>0) return false;
    ...
    if(portraitPlayMode){
      // 縦型水中：全身を泡で包み、約0.30秒で大きく上下へ移動。
      const dir=verticalDir || (input.y<0 ? 'up':'down');
      const sign=dir==='up'?-1:1;
      const other=f.isPlayer?enemy:player;
      const sideShift=other?Math.max(-40,Math.min(40,(other.x-f.x)*.12)):0;

      f.specialT=.42;
      f.raphaelMoveDuration=.30;
      f.raphaelMoveControlX=f.x+sideShift*.3;
      f.raphaelMoveControlY=f.y+sign*Math.min(175,innerHeight*.18);
      f.raphaelMoveEndX=Math.max(50,Math.min(innerWidth-50,f.x+sideShift));
      f.raphaelMoveEndY=Math.max(70,Math.min(innerHeight-105,f.y+sign*Math.min(430,innerHeight*.40)));
      f.vx=0;
      f.vy=0;
  ...
  function isKokabielForm(f){
    return !!f && (f.type==='kokabiel'||f.type==='awakenedKokabiel');
  }
  function specialGravityBall(f){
    if(gameOver||!isKokabielForm(f)||f.stun>0||f.guard||f.specialT>0||f.attackT>0)return false;
    const awakened=f.type==='awakenedKokabiel';
    const target=f.isPlayer?enemy:player;
    ...
    const speed=awakened?205:185, spreads=awakened?[-.14,0,.14]:[0];
    spreads.forEach(a=>{
      const ca=Math.cos(a),sa=Math.sin(a),ax=nx*ca-ny*sa,ay=nx*sa+ny*ca;
      gravityBalls.push({owner:f,x:f.x+ax*48,y:f.y+ay*48-8,vx:ax*speed,vy:ay*speed,
        r:awakened?23:20,t:4.4,damage:awakened?5.2:4.4,reflects:0,maxReflect:4,
        pull:awakened?310:210,awakened});
    });
    comboEl.textContent=awakened?'強化グラビティボール×3!':'グラビティボール…';return true;
  }
  function specialGravityZone(f){
    if(gameOver||!isKokabielForm(f)||f.stun>0||f.specialT>0)return false;
    const awakened=f.type==='awakenedKokabiel';
    ...
    gravityZones.push({owner:f,x,y,r:22,maxR:awakened?235:132,t:awakened?6.2:3.2,
      life:awakened?6.2:3.2,arm:awakened?.34:.42,power:awakened?1.85:1,awakened});
    comboEl.textContent=awakened?'超重力グラビティゾーン!':'グラビティゾーン…';return true;
  }
  function specialMeteorRain(f){
    if(gameOver||!isKokabielForm(f)||f.stun>0||f.guard||f.specialT>0||f.attackT>0)return false;
    const t=f.isPlayer?enemy:player;if(!t)return false;
    const awakened=f.type==='awakenedKokabiel';
    f.specialType='meteorRain';f.specialT=awakened?1.02:.85;f.attack='punch';f.attackT=.42;
    const base=t.x,offs=awakened?[-150,-112,-76,-40,0,38,74,110,148,-58,58]:[-90,-38,35,82,0];
    offs.forEach((ox,i)=>meteorDrops.push({owner:f,x:Math.max(45,Math.min(innerWidth-45,base+ox)),
      y:-45-(i%6)*22,vy:(awakened?285:265)+(i%5)*16,r:(awakened?19:18)+(i%2)*3,
      delay:.12+i*(awakened?.075:.13),t:awakened?3.1:2.4,active:false,damage:awakened?4.5:4}));
    comboEl.textContent=awakened?'強化メテオレイン!':'メテオレイン…';return true;
  }
  function specialGravityDive(f){
    if(gameOver||!isKokabielForm(f)||f.stun>0||f.guard||f.specialT>0||f.attackT>0)return false;
    const awakened=f.type==='awakenedKokabiel';
    const target=f.isPlayer?enemy:player;
    const sx=target?(Math.sign(target.x-f.x)||f.face):f.face;
    f.face=sx;f.specialType='gravityDive';f.specialT=awakened?1.02:.58;f.attack='kick';f.attackT=awakened?1.02:.58;
    f.gravityDiveHit=false;f.gravityDiveAwakened=awakened;
    f.vx=sx*(awakened?405:285);f.vy=awakened?455:430;
    comboEl.textContent=awakened?'強化グラビティダイブ!':'グラビティダイブ!';return true;
  }
  ...
  function remielActiveMirages(f){
    return remielMirages.filter(m=>m.owner===f&&m.t>0);
  }
  function remielActiveMirage(f){
    return remielActiveMirages(f)[0]||null;
  }
  function remielGhostPos(m){
    if(!m||!m.owner)return null;
    const p=Math.max(0,Math.min(1,(m.age||0)/.20));
    const e=p*p*(3-2*p);
    return {
      x:m.owner.x+(m.offsetX||0)*e,
      y:m.owner.y+(m.offsetY||0)*e
    };
  }
  ...
  function specialRemielFrostShot(f){
    ...
    if(target)mirages.forEach(mir=>{
      const g=remielGhostPos(mir),dx0=target.x-g.x,face=dx0>=0?1:-1;
      const sx=g.x+face*42,sy=g.y-12,dx=target.x-sx,dy=target.y-sy,d=Math.hypot(dx,dy)||1,sp=265;
      remielFakeShots.push({owner:f,mirage:mir,x:sx,y:sy,vx:dx/d*sp,vy:dy/d*sp,r:14,t:1.15,life:1.15,damage:2.4*f.damageMul,hit:false});
    });
    return true;
  }
  ...
  function specialSamaelTongueShot(f){
    ...
    const target=f.isPlayer?enemy:player;
    if(!target) return false;
    if(Math.abs(target.x-f.x)>3) f.face=target.x>=f.x?1:-1;
  ...
  function gabrielJetRise(f){
    ...
    f.guard=false;
    f.gabJetT=.46;
    f.gabJetUpperReady=true;
    f.specialType='gabrielJetRise';
    f.specialT=.62;
    f.attack=null;f.attackT=0;
    f.vx*=.15;
    f.vy=-600;
    comboEl.textContent='ウォータージェット上昇!';
    setTimeout(()=>{if(comboEl.textContent==='ウォータージェット上昇!')comboEl.textContent='';},560);
    clearCommand();
    return true;
  }
  ...
  function gabrielCloudBarrage(f){
    if(gameOver||!f||f.type!=='blue'||f.stun>0||f.specialT>0)return false;
    const target=gabrielTarget(f);
    if(!target)return false;
    f.specialType='gabrielCloudBarrage';
    f.specialT=.92;
    comboEl.textContent='クラウドレインショット!';

    for(let i=0;i<32;i++){
      setTimeout(()=>{
        if(gameOver||!target)return;
        // 最初から流れている背景雲の位置をイメージし、画面上部〜中段の
        // 左右端から水弾を撃ち込む。ミストクラウドとは連動しない。
        const fromLeft=i%2===0;
        const sx=fromLeft?-24:innerWidth+24;
        const cloudBand=[.24,.42,.60][i%3];
        const sy=Math.max(70,Math.min(innerHeight-130,innerHeight*cloudBand+(Math.random()-.5)*46));
        ...
        const shot={
          ...
          style:'aqua',poisonDuration:0,curve:0,wobble:.02,
          baseVy:dy/len*speed,maxReflect:0
        };
        water2Shots.push(shot);
      },i*38);
    }
    setTimeout(()=>{if(comboEl.textContent==='クラウドレインショット!')comboEl.textContent='';},1250);
    clearCommand();
    return true;
  }

  function gabrielFiveWay(f){
    if(gameOver||!f||f.type!=='blue'||f.stun>0||f.specialT>0)return false;
    const target=gabrielTarget(f);
    if(!target)return false;
  ...
  function raphaelAirCutterEscape(f,dirName){
    ...
    water2Shots.push({owner:f,x:f.x+shotDir.x*30,y:f.y+shotDir.y*30,vx:shotDir.x*300,vy:shotDir.y*300,r:18,age:0,maxAge:4,t:1,life:1,damage:3.4,name:'エアカッター',color:'aqua',reflected:0,hit:false,spin:0,style:'crescentAir',poisonDuration:0,curve:0,wobble:.01,baseVy:shotDir.y*300,maxReflect:2,raphaelStrongHoming:true});
  ...
  function trySpecial(f,kind){
    ...
    // 雲上格闘2：ミカエル。基本技は1方向＋ボタン、サイクロンだけ上位コマンド。
    if(f.type==='green'){
    ...
    if(f.type==='yellow' && portraitPlayMode){
      ...
      // 上下＋キック：泡に包まれて高速上下移動
      if(kind==='kick' && rUp) return specialRaphaelBubbleMove(f,'up');
      if(kind==='kick' && rDown) return specialRaphaelBubbleMove(f,'down');

      // 上＋パンチ：大型のエア（水圧）ブレード
      if(kind==='punch' && rUp){
        const v=rafaelTargetVector(f);
        clearCommand();
        return specialWater2Shot(f,{
          name:'エア（水圧）ブレード',attack:'punch',
          color:'water',style:'cutter',speed:300,damage:5.5,r:30,
          charge:.22,wobble:.02,maxReflect:3,aimX:v.x,aimY:v.y
        });
      }

      // 下＋パンチ：画面上方から相手位置へ落ちるエアギロチン
      if(kind==='punch' && rDown){
        return specialRafaelVerticalGuillotine(f);
      }
      ...
      if(kind==='punch' && rForward){
        ...
        clearCommand();
        return specialWater2Shot(f,{
          name:'水圧カッター',attack:'punch',
          color:'water',style:'cutter',speed:345,damage:4.2,r:19,
          charge:.14,wobble:.01,maxReflect:2,aimX:v.x,aimY:v.y
        });
      }
    }
  ...
  function attack(f, kind) {
    ...
    if(kind==='punch'){
      ...
      // 通常の舌。発動時に必ず相手へ向き直り、上下差も含めて直接狙う。
      playSfx('tongue');
      if(other && Math.abs(other.x-f.x)>3) f.face=other.x>=f.x?1:-1;
      f.tongueT=.22;
      f.attack='tongue';
      f.attackT=.3;

      const tongueDist=Math.hypot(other.x-f.x,other.y-f.y);
      if(tongueDist<f.tongueRange){
        f.bossTongueAimY=other.y-f.y;
        setTimeout(()=>{
          if(!other.guard){
            // まず小ダメージ
    ...
    const down=e=>{
      ...
      if(action==='guard'){
        if(player){
          ...
          if(player.type==='kokabiel'||player.type==='awakenedKokabiel'){
          ...
          // ラファエル旧入力：横型水中だけ上＋ガードで高速バブル移動。
          // 縦型では上/下＋キックに統一。
          if(player.type==='yellow' && !portraitPlayMode && !player.throwState && input.y<-.35){
            input.simpleGuardTapTimes=[];
...
function drawBackground(dt){
  ...
  function loop(now){
    ...
    if(!gameOver){
      ...
      [player,enemy].forEach(f=>{
        ...
        if(!isKokabielForm(f)||f.specialType!=='gravityDive'||f.specialT<=0)return;
        const awakened=!!f.gravityDiveAwakened;
        f.vy=Math.max(f.vy,awakened?420:390);
        if(awakened)f.vx=f.face*Math.max(Math.abs(f.vx),365);
        const o=f.isPlayer?enemy:player;
        if(o&&!f.gravityDiveHit&&Math.abs(o.x-f.x)<(awakened?92:78)&&Math.abs(o.y-f.y)<(awakened?96:82)){
          f.gravityDiveHit=true;
          damageHit(f,o,(awakened?10.4:8.2)*f.damageMul,(awakened?205:145)*f.face,awakened?175:135);
          o.gravityHeavyT=awakened?4.2:2.25;o.vy=Math.max(o.vy,awakened?235:180);
          spawnImpact(o.x,o.y,'hit');comboEl.textContent=awakened?'超重力ダイブ!':'ヘヴィ・グラビティ!';
        }
      });
      ...
      gravityZones.forEach(z=>{
        ...
        if(z.arm<=0&&d<z.maxR){const force=((1-d/z.maxR)*520+95)*(z.power||1);target.vx+=dx/d*force*dt;target.vy+=dy/d*force*.78*dt;}
      });
    ...
    gravityZones.forEach(z=>{
      ...
      const armBase=z.awakened?.34:.42;const rr=z.arm>0?22+(z.maxR-22)*(1-z.arm/armBase):z.maxR;
  ...
  if(mixBattleMode && mixBattleContext){
    ...
    requestAnimationFrame(()=>{
      ...
      if(player && enemy){
        const playerSide=playerIsAttacker?mixBattleContext.attackerSide:mixBattleContext.defenderSide;
        const enemySide=playerIsAttacker?mixBattleContext.defenderSide:mixBattleContext.attackerSide;
        if(playerSide)player.wingKind=playerSide==='demon'?'demon':'angel';
        if(enemySide)enemy.wingKind=enemySide==='demon'?'demon':'angel';
      }
